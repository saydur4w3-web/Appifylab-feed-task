...
export const MobileBottomNav: FC = () => {
  ...
  const messengerRef = useRef<HTMLLIElement>(null)
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (messengerRef.current && !messengerRef.current.contains(e.target as Node)) setShowMessenger(false)
  ...
  return (
    <div className="_mobile_navigation_bottom_wrapper d-lg-none">
          ...
          <div className="row">
            <div className="col-12">
              <ul className="_mobile_navigation_bottom_list">
                ...
                <li className="_mobile_navigation_bottom_item">
                  <a href="/feed" className="_mobile_navigation_bottom_link _mobile_navigation_bottom_link_active">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="27" fill="none" viewBox="0 0 24 27">
                      <path fill="#0866FF" stroke="#0866FF" strokeWidth="1.5" d="M1 13.042c0-2.094 0-3.141.431-4.061.432-.92 1.242-1.602 2.862-2.965l1.571-1.321C8.792 2.232 10.256 1 12 1c1.744 0 3.208 1.232 6.136 3.695l1.572 1.321c1.62 1.363 2.43 2.044 2.86 2.965.432.92.432 1.967.432 4.06v6.54c0 2.908 0 4.362-.92 5.265-.921.904-2.403.904-5.366.904H7.286c-2.963 0-4.445 0-5.365-.904C1 23.944 1 22.49 1 19.581v-6.54z" />
                ...
                {/* Notifications */}
                <li className="_mobile_navigation_bottom_item position-relative" ref={notifyRef}>
                  <div
                ...
                {/* Messenger */}
                <li className="_mobile_navigation_bottom_item position-relative" ref={messengerRef}>
                  <div
                    className="_mobile_navigation_bottom_link"
                    style={{ cursor: 'pointer' }}
                  ...
                  {showMessenger && (
                    <div className="position-fixed start-0 end-0 bottom-0 bg-white shadow-lg" style={{ zIndex: 9999, height: '80vh', borderRadius: '20px 20px 0 0' }}>
                      <div className="p-3 border-bottom d-flex justify-content-between">
                        <h5>Messages</h5>
                        <button onClick={() => setShowMessenger(false)} className="btn-close"></button>
                      </div>
                ...
                {/* Menu (Profile) */}
                <li className="_mobile_navigation_bottom_item position-relative" ref={profileRef}>
                  <div
                    className="_mobile_navigation_bottom_link"
                    style={{ cursor: 'pointer' }}
                  ...
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="14" fill="none" viewBox="0 0 18 14">
                      <path stroke={showProfileMenu ? '#0866FF' : '#666'} strokeLinecap="round" strokeWidth="1.5" d="M1 1h16M1 7h16M1 13h16" />
                    </svg>
                  </div>
                  {showProfileMenu && (
                    <div className="position-fixed start-0 end-0 bottom-0 bg-white shadow-lg" style={{ zIndex: 9999, height: '80vh', borderRadius: '20px 20px 0 0' }}>
                      <div className="p-3 border-bottom d-flex justify-content-between">
                        <h5>Menu</h5>
                        <button onClick={() => setShowProfileMenu(false)} className="btn-close"></button>
                      </div>
                      <div className="p-3">
                        <div className="d-flex align-items-center mb-4">
                          <img src={user?.profile_img || DEFAULT_IMG_USER} style={{width: '60px'}} alt="Profile" className="rounded-circle me-3" width="60" />
                          <div>
                            <h6 className="mb-0"> {user?.first_name} {user?.last_name} </h6>
                            <a href="/profile" className="text-muted small">View Profile</a>
                          </div>
                        </div>
                        <hr />
                        <ul className="list-unstyled">
                          <li className="py-2"><a href="#0" className="text-dark">Settings & Privacy</a></li>
                          <li className="py-2"><a href="#0" className="text-dark">Help & Support</a></li>
                          <li className="py-2"
                          onClick={logout}
                          ><span className="text-dark">Log Out</span></li>
                        </ul>
                      </div>
                    </div>
                  )}
